 ...
 */
function palermo_sanitize_checkbox( $input ) {
	if ( $input == 1 ) {
		return 1;
	}
 ...
 * is not checked, palermo_sanitize_checkbox() throws an error.
 * palermo_sanitize_checkbox_ref() however evaluates &$input as null so no errors are thrown.
 *
 * @param int|string|bool &$input Input value to sanitize.
 * @return int|string Returns 1 if $input evaluates to 1, an empty string otherwise.
 */
function palermo_sanitize_checkbox_ref( &$input ) {
	if ( $input == 1 ) {
		return 1;
	}
 ...
 * @return string
 */
function palermo_sanitize_pagination_method( $option ) {
 ...
 */
function palermo_sanitize_intval_or_empty( $input ) {
 ...
 */
function palermo_sanitize_hex_color( $str, $return_hash = true, $return_fail = '' ) {
 ...
 */
function palermo_get_allowed_tags( $context = '' ) {
	$allowed = array(
		'a'       => array(
			'href'   => true,
			'title'  => true,
	...
	return apply_filters( 'palermo_get_allowed_tags', $allowed, $context );
}
 ...
 * @return string
 */
function palermo_sanitize_custom_css( $string ) {
