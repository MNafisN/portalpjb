<?php
if ( ! function_exists( 'palermo_color_luminance' ) ) :
	/**
	 ...
	 */
	function palermo_color_luminance( $color, $percent ) {
		// Remove # if provided
		if ( '#' === $color[0] ) {
...
if ( ! function_exists( 'palermo_hex2rgba' ) ) :
	/**
	 ...
	 * @return string
	 */
	function palermo_hex2rgba( $color, $opacity = false ) {
...
if ( ! function_exists( 'palermo_posts_pagination' ) ) :
	/**
	 ...
	 * @return void
	 */
	function palermo_posts_pagination( $args = array(), WP_Query $query = null ) {
		$args = wp_parse_args( $args, apply_filters( 'palermo_posts_pagination_default_args', array(
			'mid_size'           => 1,
			'prev_text'          => _x( 'Previous', 'previous post', 'palermo' ),
			'next_text'          => _x( 'Next', 'next post', 'palermo' ),
			'screen_reader_text' => __( 'Posts navigation', 'palermo' ),
			'container_id'       => '',
			'container_class'    => '',
		), $query ) );
