<?php
add_action( 'after_setup_theme', 'palermo_setup_helpers_post_meta' );
function palermo_setup_helpers_post_meta() {
	add_image_size( 'palermo_featgal_small_thumb', 100, 100, true );

	add_action( 'wp_ajax_palermo_featgal_AJAXPreview', 'palermo_featgal_AJAXPreview' );
}

add_action( 'admin_enqueue_scripts', 'palermo_admin_register_post_meta_scripts' );
function palermo_admin_register_post_meta_scripts( $hook ) {
	$theme = wp_get_theme();

	wp_register_style( 'palermo-post-meta', get_template_directory_uri() . '/inc/css/post-meta.css', array(), $theme->get( 'Version' ) );
	wp_register_script( 'palermo-post-meta', get_template_directory_uri() . '/inc/js/post-meta.js', array(
		'media-editor',
		'jquery',
		'jquery-ui-sortable'
	), $theme->get( 'Version' ) );
	...
	$settings = array(
		...
		'tSelectFile'         => esc_html__( 'Select file', 'palermo' ),
		'tSelectFiles'        => esc_html__( 'Select files', 'palermo' ),
		'tUseThisFile'        => esc_html__( 'Use this file', 'palermo' ),
		'tUseTheseFiles'      => esc_html__( 'Use these files', 'palermo' ),
		'tUpdateGallery'      => esc_html__( 'Update gallery', 'palermo' ),
		'tLoading'            => esc_html__( 'Loading...', 'palermo' ),
		'tPreviewUnavailable' => esc_html__( 'Gallery preview not available.', 'palermo' ),
		'tRemoveImage'        => esc_html__( 'Remove image', 'palermo' ),
		'tRemoveFromGallery'  => esc_html__( 'Remove from gallery', 'palermo' ),
	);
	wp_localize_script( 'palermo-post-meta', 'palermo_PostMeta', $settings );
}
...
function palermo_prepare_metabox( $post_type ) {
	wp_nonce_field( basename( __FILE__ ), $post_type . '_nonce' );
}

function palermo_can_save_meta( $post_type ) {
	global $post;
...
function palermo_metabox_gallery( $gid = 1 ) {
	global $post;
	$post_id = $post->ID;

	palermo_featgal_print_meta_html( $post_id, $gid );
}

function palermo_metabox_gallery_save( $POST, $gid = 1 ) {
	global $post;
	$post_id = $post->ID;

	palermo_featgal_update_meta( $post_id, $POST, $gid );
}

function palermo_metabox_input( $fieldname, $label, $params = array() ) {
	global $post;
...
function palermo_metabox_textarea( $fieldname, $label, $params = array() ) {
	global $post;
...
function palermo_metabox_dropdown( $fieldname, $options, $label, $params = array() ) {
	global $post;
...
function palermo_metabox_radio( $fieldname, $optionname, $optionval, $label, $params = array() ) {
	global $post;
...
function palermo_metabox_checkbox( $fieldname, $value, $label, $params = array() ) {
	global $post;
...
function palermo_metabox_open_tab( $title ) {
	?>
...
function palermo_metabox_close_tab() {
	?>
		</div>
	</div>
	<?php
...
function palermo_metabox_open_collapsible( $title ) {
	?>
	<div class="postbox" style="margin-top:20px">
		<div class="handlediv" title="<?php esc_attr_e( 'Click to toggle', 'palermo' ); ?>"><br></div>
		<h3 class="hndle"><?php echo esc_html( $title ); ?></h3>
		<div class="inside">
	<?php
}

function palermo_metabox_close_collapsible() {
	?>
		</div>
	</div>
	<?php
...
function palermo_metabox_guide( $strings, $params = array() ) {
	$defaults = array(
...
function palermo_bind_metabox_to_page_template( $metabox_id, $template_file, $js_var ) {
...
ENDJS;
	...
	palermo_add_inline_js( $js, sanitize_key( 'metabox_template_' . $metabox_id . '_' . $template_file ) );
}

function palermo_bind_metabox_to_post_format( $metabox_id, $post_format, $js_var ) {
...
ENDJS;
	...
	palermo_add_inline_js( $js, sanitize_key( 'metabox_format_' . $metabox_id . '_' . $post_format ) );
}
 ...
 * @return void
 */
function palermo_featgal_print_meta_html( $post_id = false, $gid = 1 ) {
	if ( $post_id == false ) {
		global $post;
		$post_id = $post->ID;
	}
	...
	<div class="ci-media-manager-gallery">
		<input type="button" class="ci-upload-to-gallery button" value="<?php esc_attr_e( 'Add Images', 'palermo' ); ?>"/>
		...
		<p><label class="ci-upload-to-gallery-random"><input type="checkbox" name="ci_featured_gallery_rand_<?php echo esc_attr( $gid ); ?>" value="rand" <?php checked( $rand, 'rand' ); ?> /> <?php esc_html_e( 'Randomize order', 'palermo' ); ?></label></p>
		<div class="ci-upload-to-gallery-preview group">
			<?php
				$images = palermo_featgal_get_images( $ids );
				if ( $images !== false and is_array( $images ) ) {
					foreach ( $images as $image ) {
						?>
						<div class="thumb">
							<img src="<?php echo esc_url( $image['url'] ); ?>" data-img-id="<?php echo esc_attr( $image['id'] ); ?>">
							<a href="#" class="close media-modal-icon" title="<?php esc_attr_e( 'Remove from gallery', 'palermo' ); ?>"></a>
						</div>
						<?php
					}
				}
			?>
			<p class="ci-upload-to-gallery-preview-text"><?php esc_html_e( 'Your gallery images will appear here', 'palermo' ); ?></p>
		</div>
	</div>
	<?php
}
 ...
 */
function palermo_featgal_update_meta( $post_id, $POST, $gid = 1 ) {
	if ( absint( $post_id ) < 1 ) {
		return false;
	}
...
function palermo_featgal_get_ids( $post_id = false, $gid = 1 ) {
	if ( $post_id == false ) {
		global $post;
		$post_id = $post->ID;
	} else {
...
function palermo_featgal_get_attachments( $post_id = false, $gid = 1, $extra_args = array() ) {
	if ( $post_id == false ) {
		global $post;
		$post_id = $post->ID;
	} else {
 ...
 * Hooked to wp_ajax_palermo_featgal_AJAXPreview for AJAX updating of the galleries' previews.
 */
function palermo_featgal_AJAXPreview() {
	$ids  = $_POST['ids'];
	$urls = palermo_featgal_get_images( $ids );
	if ( $urls === false ) {
		echo 'FAIL';
 ...
 * @return array|bool
 */
function palermo_featgal_get_images( $csv = false ) {
	$ids = explode(',', $csv);
	$ids = array_filter($ids);
	...
	if ( count( $ids ) > 0 ) {
		...
		foreach ( $ids as $id ) {
			$thumb_file = palermo_get_image_src( $id, 'palermo_featgal_small_thumb' );

			$file = parse_url( $thumb_file );
			$file = pathinfo( $file['path'] );
			$file = basename( $file['basename'], '.' . $file['extension'] );

			$size = $image_sizes['palermo_featgal_small_thumb']['width'] . 'x' . $image_sizes['palermo_featgal_small_thumb']['height'];
			if ( palermo_substr_right( $file, strlen( $size ) ) == $size ) {
				$file = $thumb_file;
			} else {
				$file = palermo_get_image_src( $id, 'thumbnail' );
			}

			$data = array(
				'id'  => $id,
				//'url' => ci_get_image_src($id, 'palermo_featgal_small_thumb')
				'url' => $file
			);
