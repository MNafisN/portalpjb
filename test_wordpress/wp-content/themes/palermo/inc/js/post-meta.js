jQuery( document ).ready( function( $ ) {
	...
	$( 'body' ).on( 'click', '.ci-media-button', function( e ) {
		...
		var ciMediaUpload = wp.media( {
			...
			title: bMulti == true ? palermo_PostMeta.tSelectFiles : palermo_PostMeta.tSelectFile,
			button: {
				text: bMulti == true ? palermo_PostMeta.tUseTheseFiles : palermo_PostMeta.tUseThisFile
			},
			multiple: bMulti
		} ).on( 'select', function(){
				...
				*/
				...
				if( bMulti == false ) {
					...
					if( target_preview.length > 0 ) {
						...
						// Only 'thumbnail', 'medium' and 'full' are exposed, so we use 'thumbnail' instead of 'palermo_featgal_small_thumb'
						var html = '<img src="' + attachment.sizes.thumbnail.url + '" /><a href="#" class="close media-modal-icon" title="' + palermo_PostMeta.tRemoveImage + '"></a>';
	...
	$( 'body' ).on( 'click', '.ci-upload-to-gallery', function( e ) {
		...
		var ciMediaGallery = wp.media( {
			frame: 'select',
			title: palermo_PostMeta.tSelectFiles,
			button: {
				text: palermo_PostMeta.tUseTheseFiles
			},
			multiple: true
			...
			palermo_featgal_AJAXUpdate( target_parent );
	...
	function palermo_featgal_UpdateIDs( preview_element ) {
		var ids = [];
	...
	function palermo_featgal_AJAXUpdate( gallery_container ) {
		...
		$.ajax( {
			type: 'post',
			url: palermo_PostMeta.ajaxurl,
			data: {
				action: 'palermo_featgal_AJAXPreview',
				ids: target_ids.val()
			},
			dataType: 'text',
			beforeSend: function() {
				target_preview.empty().html( '<p>' + palermo_PostMeta.tLoading + '</p>' );
			},
			success: function( response ) {
				if ( response == 'FAIL' ) {
					target_preview.empty().html( '<p>' + palermo_PostMeta.tPreviewUnavailable + '</p>' );
				} else {
					...
					$.each( response, function( key, value ) {
						$( '<div class="thumb"><img src="' + value.url + '" data-img-id="' + value.id + '"><a href="#" class="close media-modal-icon" title="' + palermo_PostMeta.tRemoveFromGallery + '"></a></div>' ).appendTo( target_preview );
					} );
				}
			}//success
		});//ajax
	...
	$( 'body' ).on( 'click', '.ci-media-manager-gallery .thumb a.close', function( event ) {
		...
		palermo_featgal_UpdateIDs( container );
	} );
	...
	if ( preview_areas.length > 0 ) {
		preview_areas.sortable( {
			update: function( event, ui ) {
				palermo_featgal_UpdateIDs( $( this ) );
			}
		} );
	}
