<?php
function palermo_admin_notice_sample_content() {
	...
	?>
	<div class="notice notice-info is-dismissible palermo-sample-content-notice">
		<p>
			<?php echo wp_kses( sprintf( __( 'It looks like the theme was just installed. <a href="%s" target="_blank">Download the sample content</a> to get things moving.', 'palermo' ),
	...
	wp_enqueue_script( 'palermo-sample-content', get_template_directory_uri() . '/inc/js/sample-content.js', array(), $theme->get( 'Version' ), true );

	$settings = array(
		'ajaxurl'       => admin_url( 'admin-ajax.php' ),
		'dismiss_nonce' => wp_create_nonce( 'palermo-dismiss-sample-content' ),
	);
	wp_localize_script( 'palermo-sample-content', 'palermo_SampleContent', $settings );
}

function palermo_ajax_dismiss_sample_content() {
	check_ajax_referer( 'palermo-dismiss-sample-content', 'nonce' );
...
function palermo_wrap_archive_widget_post_counts_in_span( $output ) {
	$output = preg_replace_callback( '#(<li>.*?<a.*?>.*?</a>.*)&nbsp;(\(.*?\))(.*?</li>)#', 'palermo_replace_archive_widget_post_counts_in_span', $output );

	return $output;
}

function palermo_replace_archive_widget_post_counts_in_span( $matches ) {
...
function palermo_wrap_category_widget_post_counts_in_span( $output, $args ) {
	if ( ! isset( $args['show_count'] ) || $args['show_count'] == 0 ) {
		return $output;
	}
	$output = preg_replace_callback( '#(<a.*?>\s*)(\(.*?\))#', 'palermo_replace_category_widget_post_counts_in_span', $output );

	return $output;
}

function palermo_replace_category_widget_post_counts_in_span( $matches ) {
 ...
 */
function palermo_get_page_var( $default_return = 0 ) {
 ...
 */
function palermo_get_image_src( $image_id, $size = 'full' ) {
 ...
 * @return string
 */
function palermo_substr_left( $string, $length ) {
 ...
 * @return string
 */
function palermo_substr_right( $string, $length ) {
	return substr( $string, - $length, $length );
}

if ( ! function_exists( 'palermo_get_related_posts' ) ):
/**
 ...
 */
function palermo_get_related_posts( $post_id, $related_count, $args = array() ) {
...
$palermo_glob_inline_js = array();
if ( ! function_exists( 'palermo_add_inline_js' ) ):
/**
 ...
 */
function palermo_add_inline_js( $script, $handle = false ) {
	global $palermo_glob_inline_js;

	$handle = sanitize_key( $handle );

	if ( ( $handle !== false ) and ( $handle != '' ) ) {
		$palermo_glob_inline_js[ $handle ] = "\n" . $script . "\n";
	} else {
		$palermo_glob_inline_js[] = "\n" . $script . "\n";
	}
}
endif;

if ( ! function_exists( 'palermo_get_inline_js' ) ):
/**
 * Retrieves the inline JS scripts that are registered for printing.
 *
 * @return array The inline JS scripts queued for printing.
 */
function palermo_get_inline_js() {
	global $palermo_glob_inline_js;

	return $palermo_glob_inline_js;
}
endif;

if ( ! function_exists( 'palermo_print_inline_js' ) ):
/**
 * Prints the inline JS scripts that are registered for printing, and removes them from the queue.
 */
function palermo_print_inline_js() {
	global $palermo_glob_inline_js;

	if ( empty( $palermo_glob_inline_js ) ) {
		return;
	}

	$sanitized = array();

	foreach ( $palermo_glob_inline_js as $handle => $script ) {
...
if ( ! function_exists( 'palermo_inflect' ) ):
	/**
	 ...
	 * @return string
	 */
	function palermo_inflect( $num, $none, $one, $many ) {
		if ( $num == 0 ) {
			return $none;
		} elseif ( $num == 1 ) {
			return $one;
...
if ( ! function_exists( 'palermo_e_inflect' ) ):
	/**
	 ...
	 * @return void
	 */
	function palermo_e_inflect( $num, $none, $one, $many ) {
		echo palermo_inflect( $num, $none, $one, $many );
	}
endif;

if ( ! function_exists( 'palermo_merge_wp_queries' ) ):
/**
 ...
 */
function palermo_merge_wp_queries() {
	$args = func_get_args();
	...
	$params = apply_filters( 'palermo_merge_wp_queries', $params, $args );
...
if ( ! function_exists( 'palermo_dropdown_posts' ) ):
/**
 ...
 */
function palermo_dropdown_posts( $args = '', $name = 'post_id' ) {
	$defaults = array(
		'depth'                 => 0,
		'post_parent'           => 0,
		'selected'              => 0,
	...
	$output = apply_filters( 'palermo_dropdown_posts', $output, $name, $r );
 ...
 */
function palermo_can_use_plugin( $plugin ) {
